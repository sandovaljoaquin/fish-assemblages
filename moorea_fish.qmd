...
ggplot(full_data_frame, 
       aes(x = proportion_corallivore, 
           y = mean_site_coral_cover, 
           color = year)) + 
  geom_point() + 
  facet_wrap("site")
```
...
```{r}
model <- betareg(
  proportion_corallivore ~ mean_site_coral_cover + site + year,
...
```{r}
pred_grid <- expand_grid(proportion_corallivore = seq(0,1, by = 0.01), 
                         site_coral_cover = seq(0,1, by = 0.01))

pred_grid <- pred_grid |> 
mutate(expected = predict(object = model, 
                            newdata = pred_grid, 
                            type = "response"))
