...
## Total Counts of Each Fine Trophic Level Group in 2006 and 2024.
...
fish_2024_plot
```
Corallivores fell from the third‑most abundant group in the first survey year (202 individuals) to the sixth‑most abundant group in the most recent sampling year (79 individuals) at the LTER Backreef sites.
...
ggplot(full_data_frame, 
  ...
  labs(x = "Mean Site Coral Cover", 
       ...
       color = "Year")
```
** Need to decide to use betareg() or glmmTMB()** 
- Having issues generating CI. Manually retrieve SE values? Only possible with glmmTMB() so far.
